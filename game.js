...
['cinema',0,3000,400,300,'#2a1f2e','CINEMA',.35],['books',500,3000,400,300,'#5a4636','BOOKSHOP',.1],['arcade',1000,3000,400,300,'#1c1f3a','ARCADE',.2],['noodle',1500,3000,400,300,'#6b4a3a','NOODLE SHOP',0]];
...
const MIX={forest:[.03,.03,900],future:[0,0],cafe:[.04,.02,3500],city:[.04,.015,1500],beach:[.03,.07,600],garden:[.04,.01,1200],home:[.05,0,1200],pet:[.04,.01,1200],bedroom:[.012,0,1200],station:[.04,.02,900],lighthouse:[.03,.07,600],orchard:[.04,.03,1400],market:[.05,.02,2000],cinema:[.05,0,900],books:[.04,0,1200],arcade:[.05,.01,3000],noodle:[.05,.01,1500]};
...
pan(`<h2>WORLD COMPLETE</h2><p>${pc}% EXPLORED</p><p>TOTAL TIME:<br>${f(Math.floor(t/3600))}:${f(Math.floor(t/60)%60)}:${f(t%60)}</p>${S.wish?'<p>YOUR WISH:<br><i>'+esc(S.wish)+'</i></p>':''}${S.bottle?'<p>YOUR BOTTLE:<br><i>'+esc(S.bottle)+'</i></p>':''}<p>FINAL REWARD:<br>LOCKED</p><button id="cl">CLAIM REWARD</button>`);
...
document.querySelectorAll('.row').forEach(e=>e.onclick=()=>{$('mr').textContent='"'+it[e.dataset.i][2]+'"';AU.chime()});$('px').onclick=unpan}
function aPet(){const f=pet.fol=!pet.fol,n=S.c.pet=(S.c.pet||0)+1;say(f?(n==1?pet.l.concat(CFG.petName+' will follow you now.\n(Press E on them again to make them stay.)'):CFG.petName+' is following you again.'):CFG.petName+' stays. Dramatically.')}
...
function aMail(){const n=S.keys.length+S.fl.length+S.stars.length+S.tk.length,L=CFG.letters.filter((_,i)=>n>=[2,6,11,16][i]);say(L.length?L.map(t=>'✉️\n\n'+t):['Empty. Waiting for something worth delivering.','(Come back after you\'ve found a few things.)'])}
...
if(mode==='play'&&!['home','bedroom','cafe','future','cinema','books','arcade','noodle'].includes(curZ)){cx.fillStyle=`rgba(20,24,80,${.26*(1+Math.sin(T/40))/2})`;cx.fillRect(0,0,w,h)}
